...
<?php if(isset($_SESSION["admin"])){
    if(isset($_REQUEST["updateplace"])){
        if(isset($_FILES['place_image']))
        {
            ...
            echo $path;
            unlink($path);
            $sql = "UPDATE places SET name='".$_REQUEST["place"]."', image='places/".$img."' WHERE ID=".$_REQUEST["id"];
            } else {
                echo "<script>alert('Sorry, only JPG, JPEG, PNG & GIF files are allowed.');</script>";
                echo "<script>window.location='ManagePlacesUpdateDelete.php?id=".$_REQUEST['id']."&task=update"."';</script>";
            }
        }
        else
            $sql = "UPDATE places SET name='".$_REQUEST["place"]."' WHERE ID=".$_REQUEST["id"];
    ...
    if(isset($_REQUEST["task"])){
      if($_REQUEST["task"]=="delete"){
        $path="../".$_REQUEST["path"];
        unlink($path);
        $sql = "DELETE FROM places WHERE ID=".$_REQUEST["id"];
        if (mysqli_query($conn, $sql)) {
          echo "<script>window.location='ManagePlaces.php';</script>";
        } else {
    ...
    $sql = "select * from places where ID=".$_REQUEST["id"];
                    ...
                    <div class="pb-3">
                        <div class="bg-white mb-3" style="padding: 30px;">
                            <div class="d-flex mb-3">
                                <span class="text-primary text-uppercase text-decoration-none" href="">Update Place</span>
                            </div>
                            <div class="bg-white mb-3">
                                <form method="post" enctype="multipart/form-data">
                                    <div class="form-group">
                                        ...
                                        <input type="text" value="<?php echo $row["name"]; ?>" class="form-control" name="place" id="name">
                                    </div>
                                    <div class="form-group">
